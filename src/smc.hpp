...
namespace proj {
    ...
    class SMC {
        public:
            ...
            void                     init();
            void                     run();
            double                   filterParticles(unsigned step, unsigned locus);
    ...
    inline void SMC::init() {
        ...
        // Stores number of darts that hit each of the G::_nparticles in
        // multinomial sampling.
        _counts.clear();
        _counts.resize(G::_nparticles);
        
        // Create log_weights vector that stores log weight of each particle
        _log_weights.resize(G::_nparticles);
        ...
        _particles.resize(G::_nparticles, template_particle);
    ...
    inline double SMC::filterParticles(unsigned step, unsigned locus) {
        // Sanity checks
        assert(_counts.size() == G::_nparticles);
        
        vector<double> log_weights(G::_nparticles, 0.0);
        unsigned i = 0;
        ...
        _log_marg_like += log_sum_weights - log(G::_nparticles);
        if (step < G::_nloci) {
        ...
        _counts.assign(G::_nparticles, 0);
    ...
    inline void SMC::saveAllSpeciesTrees(string fnprefix, const vector<Particle> & particles, unsigned compression_level) {
        ...
        if (compression_level == 2) {
            ...
            for (auto it = tree_info.begin(); it != tree_info.end(); ++it) {
                ...
                double pct = 100.0*total_c/G::_nparticles;
            ...
            for (const Particle & p : particles) {
                ...
                if (compression_level == 1) {
                    double pct = 100.0*c/G::_nparticles;
                    ...
                    double pct = 100.0/G::_nparticles;
                    string note = "freq = 1";
