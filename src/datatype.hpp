...
namespace proj {
    ...
    inline void DataType::setGeneticCodeFromName(string genetic_code_name) {   
        assert(isCodon());
    ...
    inline string DataType::getDataTypeAsString() const {   
        string s = translateDataTypeToString(_datatype);
        if (isCodon())
            s += str(format(",%s") % _genetic_code->getGeneticCodeName());
        return s;
    }
    
    inline string DataType::translateDataTypeToString(unsigned datatype) {
