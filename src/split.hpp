...
namespace proj {
    ...
    class Split {
        ...
        public:
            ...
            typedef shared_ptr< Split >                    SharedPtr;
    };
    ...
    inline Split & Split::operator=(const Split & other) {
        ...
        _mask          = other._mask;
        _bits.resize(other._bits.size());   // necessary?
        _bits          = other._bits;
        return *this;
    }
    ...
    inline string Split::createPatternRepresentation() const {
        string s;
    ...
    inline bool Split::contains(const Split & other) const {
        // Supposed there are only 2 bits per unit
        bool viable = false;
        ...
        for (unsigned i = 0; i < _bits.size(); ++i) {
            ...
            split_unit_t ab = a & b;
            if (ab && ab == b)
                viable = true; // must be at least one unit like this
            if (ab && ab == a && ab != b)
                contradicted = true; // a contained in b in this unit
            if (!ab && b)
                contradicted = true; // b has bits set but a does not
        }
        return viable && !contradicted;
    }
}
